...
class ACO():
    ...
    def __init__(self,  # 0: depot
        ...
        self.pheromone_guide = pheromone_guide

       
        if self.pheromone_guide is None :

            self.pheromone = torch.ones(self.problem_size, self.problem_size) if pheromone is None else pheromone
        else :


             self.pheromone = self.pheromone_guide
    ...
    def gen_path_costs(self, paths:torch.Tensor):
        ...
        u = paths.permute(1, 0).cpu().numpy()  # Move the tensor to CPU and convert to NumPy
